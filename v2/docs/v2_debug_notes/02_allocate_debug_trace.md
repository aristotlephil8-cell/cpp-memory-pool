...
推理服务里类似的 allocator 快路径通常要求“本地命中时只做一次指针弹出和计数更新”。如果 miss 路径把计数搞错，会影响内存回收策略：本该保留少量本地缓存时却误判为超大缓存，可能频繁归还 CentralCache，造成锁竞争和尾延迟抖动。对在线推理而言，这类 bug 不一定立刻崩溃，但会把 P99 延迟变得难以解释。
