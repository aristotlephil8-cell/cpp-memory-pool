...
新增 cold / warm stats 后，可以更准确地区分不同层的性能现象。cold mode 通过新 worker thread 让每轮拥有新的 `thread_local ThreadCache`，因此能看到首次本地 miss、CentralCache refill 和部分 PageCache/systemAlloc 行为。warm mode 则在同一个 worker thread 中先预热，再 reset counters 并正式计数，主要观察 ThreadCache 本地 fast path。

原 stats experiment 的局限在于它排在完整 benchmark 之后，mixed small sizes 和 long stress mixed 已经被前面的测试填热，所以 hit rate 直接变成 1.000。新的 cold/warm 结果显示，mixed cold 仍会出现少量 miss 和 PageCache 申请，而 warm mode 中这些计数为 0；这说明 benchmark 状态隔离会显著影响结论。

多轮 `avg/min/max/stddev` 可以帮助判断噪声。固定 4096B refill pressure 在 cold/warm 下都保持较低 hit rate，说明瓶颈确实在 refill/CentralCache 访问频率；而 8192B cold span allocation 的耗时波动更大，说明 PageCache/systemAlloc 和全局缓存状态会影响首次路径。后续继续优化前，应优先保证 benchmark 隔离性，而不是继续调 threshold 或 batch。
